...
let TranslateInstructionToAsmSequence instruction =
    ...
    match instruction with
        ...
        | CmpEqBA               -> [ "cmp EBX,EAX" ; "setz EAX" ]
        | CmpNeBA               -> [ "cmp EBX,EAX" ; "setnz EAX" ]
        | CmpLtsBA              -> [ "cmp EBX,EAX" ; "setl EAX" ]
        | CmpLtuBA              -> [ "cmp EBX,EAX" ; "setb EAX" ]
        | CmpGtsBA              -> [ "cmp EBX,EAX" ; "setg EAX" ]
        | CmpGtuBA              -> [ "cmp EBX,EAX" ; "seta EAX" ]
        | CmpLesBA              -> [ "cmp EBX,EAX" ; "setle EAX" ]
        | CmpLeuBA              -> [ "cmp EBX,EAX" ; "setbe EAX" ]
        | CmpGesBA              -> [ "cmp EBX,EAX" ; "setge EAX" ]
        | CmpGeuBA              -> [ "cmp EBX,EAX" ; "setae EAX" ]
        | CmpAZ                 -> [ "cmp EAX,0" ; "setz EAX" ]
        | FetchLoc(r,i)         -> [ sprintf "mov %s,[@%s]" (regNameOf r) (LocalIdxNameString i) ]  // TODO: Assumes 32-bit target
        | StoreLoc(r,i)         -> [ sprintf "mov [@%s],%s" (LocalIdxNameString i) (regNameOf r) ]  // TODO: Assumes 32-bit target
